...
export function useSherpa(story: Story | null): SherpaHookResult {
  ...
  const loadWasmModule = useCallback((): Promise<void> => {
    return new Promise((resolve, reject) => {
      ...
      // [FIX] Hot-Reload Guard: Prevents 'OfflineStream' redeclaration crash
      ...
      // Check if global objects are already initialized
      ...
      const moduleConfig: SherpaModule = {
        ...
        onRuntimeInitialized: () => {
          ...
          try {
            ...
            setStatus("error");
            setStatusMessage(`ASR Initializer Failed: ${(err as Error).message}`);
            reject(err);
          }
        }
      };
  ...
  // -------------------------------------------------------------------------
  // Match results (Look-Ahead Slidding Window)
  // -------------------------------------------------------------------------
  ...
  const stop = useCallback(() => {
    ...
    if (rawMediaStreamRef.current) rawMediaStreamRef.current.getTracks().forEach(t => t.stop());
