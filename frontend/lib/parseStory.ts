...
export function parseStory(text: string, title: string = "Story"): Story {
  ...
  const paragraphs: Paragraph[] = rawParagraphs.map(
    (rawParagraph, pIdx): Paragraph => {
      const rawSentences = splitIntoSentences(rawParagraph);

      const sentences: Sentence[] = rawSentences.map(
        (rawSentence, sIdx): Sentence => {
          const rawChunks = splitIntoChunks(rawSentence);
